...
<?php
     ...
     $estado = $_POST["c_uf"];
	 $codigo_login = $_POST["c_codigo_login"];
	 $codigo_usuario = $_POST["c_codigo_usuario"];
	 $codigo_departamento = $_POST['c_departamento'];
...
<?php
	 ...
	 else if(empty($codigo_departamento))
     {
?>
          <script>
               alert("Campo DEPARTAMENTO incompleto");
               history.go(-1);
          </script>
<?php
     }
...
<?php
           ...
           else
           {
				...
				$consulta_usuarios = "SELECT * FROM $table_usuarios WHERE codigo = '$codigo_usuario'";
                $resultado_consulta_usuarios = mysqli_query($link, $consulta_usuarios);
                $vetor_usuarios = mysqli_fetch_array($resultado_consulta_usuarios);
                ...
                $vetor_logins = mysqli_fetch_array($resultado_consulta_login);
				
                $consulta_clientes = mysqli_query($link, "SELECT * FROM $table_usuarios WHERE cpf = '$cpf' AND codigo != '$codigo_usuario'");
...
<?php
                ...
                else
                {
                    ...
                    $consulta_logins = mysqli_query($link, "SELECT * FROM $table_logins WHERE nickname = '$nickname' AND codigo != '$codigo_login'");
...
<?php
                    ...
                    else
                    {
                         ...
                         $consulta_email = mysqli_query($link, "SELECT * FROM $table_logins WHERE email = '$email' AND codigo != '$codigo_login'");
...
<?php
                         ...
                         $inserir_logins = mysqli_query($link, "UPDATE $table_logins SET nickname = '$nickname' , email = '$email' , senha = '$senha' , status = 2 WHERE codigo = '$codigo_login'");
...
?>
                              <script language="JavaScript">
                                   alert("Erro ao atualizar os dados de login. Entre em contato com o desenvolvedor do sistema.");
                                   history.go(-1);
                              </script>
<?php
                         }
                         else
                         {
                              ...
                              $inserir_usuarios = mysqli_query($link, "UPDATE $table_usuarios SET nome='$nome', codigo_departamento='$codigo_departamento',sobrenome='$sobrenome',nascimento='$nasc',sexo='$sexo',telefone='$telefone',
                              celular='$celular',cpf='$cpf',codigo_login = '$vetor[0]',rua='$rua',numero='$numero',bairro='$bairro',cep='$cep',cidade='$cidade',estado='$estado' WHERE codigo = '$codigo_usuario'");
...
?>
                                   ...
                                   <script language="JavaScript">
                                        alert("Erro ao atualizar o USUÁRIO. Entre em contato com o desenvolvedor do sistema");
                                        history.go(-1);
                                   </script>
<?php
                              }
...
?>
                                   <script language="JavaScript">
                                        ...
                                        document.location.href="../tela_usuario.php";
                                   </script>
<?php
                                   }
                              }
